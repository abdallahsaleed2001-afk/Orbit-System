...
export async function getPartnerData(client, guildId) {
  ...
  // 500 and 20 were old defaults. Migrate those legacy values to the new default of 100.
  const minMembers = !Number.isFinite(storedMin) || storedMin === 500 || storedMin === 20 ? 100 : Math.max(100, storedMin);
  ...
  if (storedMin === 500 || storedMin === 20) await save(client, guildId, data);
  return data;
}
