...
export function joinMines(game, user) {
  ...
  if (isPlayer(game, user.id)) return { error: 'already' };
  if (game.players.length >= 100) return { error: 'full' };
  game.players.push(user);
  return { ok: true };
}

export function leaveMines(game, userId) {
  if (!game?.active || !isPlayer(game, userId)) return { error: 'not_in_game' };
...
export function startMines(game) {
  if (!game?.active || game.phase !== 'join') return { error: 'inactive' };
  if (game.players.length < 2) return { error: 'minimum' };
  newRound(game);
  return { ok: true, player: currentPlayer(game) };
}

export function pickMinesCell(game, userId, cell) {
  ...
  if (cell === game.mine) {
    ...
    if (game.players.length <= 1) { game.phase = 'finished'; return { mine: true, eliminated: player, winner: game.players[0] || null, finished: true }; }
    if (game.turnIndex >= game.players.length) game.turnIndex = 0;
    newRound(game);
    return { mine: true, eliminated: player, next: currentPlayer(game), finished: false };
  }
  game.turnIndex = (game.turnIndex + 1) % game.players.length;
  return { mine: false, player, next: currentPlayer(game) };
}

export function eliminateCurrentForTimeout(game) {
  if (!game?.active || game.phase !== 'turn') return null;
  const player = currentPlayer(game);
  if (!player) return null;
  game.players.splice(game.turnIndex, 1);
  game.eliminated.add(player.id);
  if (game.players.length <= 1) { game.phase = 'finished'; return { eliminated: player, winner: game.players[0] || null, finished: true }; }
  if (game.turnIndex >= game.players.length) game.turnIndex = 0;
  newRound(game);
  return { eliminated: player, next: currentPlayer(game), finished: false };
}
