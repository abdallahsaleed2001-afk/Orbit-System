...
export const cleanName=(value,max=18)=>{
...
function wheelName(p,w,h,player,cx,cy,radius,angle,count,color){
  // Keep every username visibly inside its own slice.
  const maxChars=count<=4?14:count<=6?11:count<=8?9:count<=12?7:5;
  const name=cleanName(player?.username ?? player?.displayName ?? 'PLAYER',maxChars);
  const scale=count<=6?2:1;
  const rr=radius*0.62;
  const x=Math.round(cx+Math.cos(angle)*rr);
  const y=Math.round(cy+Math.sin(angle)*rr);
  const maxWidth=Math.max(30,Math.min(radius*0.58,(2*Math.PI*radius/count)*0.82));
  const width=Math.min(maxWidth,Math.max(34,name.length*(6*scale)));
  const boxH=scale===2?18:13;
  // Dark contrast plate makes the name readable on every wheel color.
  rect(p,w,h,Math.round(x-width/2-5),Math.round(y-boxH/2),Math.round(width+10),boxH,PALETTE[14]);
  text(p,w,h,name,x,y-Math.round(3*scale),scale,color,'center',maxWidth);
}

export function drawRouletteFrame(game,selectedIndex=null,rotation=0){
  const w=700,h=500,p=Buffer.alloc(w*h*3);
  ...
  ring(p,w,h,cx,cy,r+8,PALETTE[14],10);
  ...
  // Names are rendered after all wheel graphics, so nothing can cover them.
  for(let i=0;i<count;i++){
    const player=game.participants[i];
    if(!player)continue;
    const a=-Math.PI/2+rotation+(i+0.5)*step;
  ...
  for(let y=cy-48;y<=cy+48;y++)for(let x=cx-48;x<=cx+48;x++){
    if((x-cx)**2+(y-cy)**2<=48*48)pixel(p,w,x,y,PALETTE[14]);
  }
  ring(p,w,h,cx,cy,48,white,3);
  text(p,w,h,'INFINITY',cx,cy-4,1,white,'center',90);

  // Fixed pointer.
  for(let y=32;y<63;y++){
    const half=Math.max(2,Math.floor((y-32)/4));
    for(let x=cx-half;x<=cx+half;x++)pixel(p,w,x,y,gold);
  }
  rect(p,w,h,cx-6,29,12,10,gold);

  const selected=selectedIndex!==null?game.participants[selectedIndex]:null;
  rect(p,w,h,110,454,480,24,PALETTE[14]);
  text(p,w,h,selected?`TURN: ${cleanName(selected.username,24)}`:'SPINNING...',350,461,1,selected?gold:muted,'center',450);
