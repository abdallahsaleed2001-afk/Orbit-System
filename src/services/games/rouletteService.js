...
export function beginRouletteRound(game) { if (game.participants.length < 2) return null; game.phase = 'spinning'; game.round += 1; const index = Math.floor(Math.random() * game.participants.length); game.selectedId = game.participants[index].id; return { index, participant: game.participants[index] }; }
