...
function encodeGifFrame(indices, minCodeSize = 4) {
  ...
  emit(clear);
  for (const index of indices) { emit(index); emit(clear); }
  emit(end);
...
export function createRouletteGif(game, selectedIndex) {
  const frames = 12, width = 700, height = 420;
  ...
  const start = target + Math.random() * Math.PI * 2 + Math.PI * 2 * 4.5;
  ...
  for (let frame = 0; frame < frames; frame++) {
    ...
    const encoded = encodeGifFrame(indices, 4);
