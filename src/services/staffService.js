...
export async function getStaffData(guildId) {
  return normalize(await getFromDb(key(guildId), DEFAULTS));
}

export async function saveStaffData(guildId, data) {
...
export async function getStaffProfile(guildId, userId, defaults = {}) {
  ...
  return {
    userId,
    joinedAt: existing.joinedAt || defaults.joinedAt || new Date().toISOString(),
    ...
    demotions: Array.isArray(existing.demotions) ? existing.demotions : [],
    notes: Array.isArray(existing.notes) ? existing.notes : [],
    activity: existing.activity && typeof existing.activity === 'object' ? existing.activity : {},
    ...existing,
  };
}

export async function ensureStaffMember(guildId, userId, defaults = {}) {
  const data = await getStaffData(guildId);
  const profile = await getStaffProfile(guildId, userId, defaults);
  data.members[userId] = profile;
  await saveStaffData(guildId, data);
  return profile;
}

export async function updateStaffConfig(guildId, patch) {
  const data = await getStaffData(guildId);
  data.config = { ...data.config, ...patch };
  return saveStaffData(guildId, data);
}
...
export async function removeStaffWarning(guildId, userId, warningId) {
  const data = await getStaffData(guildId);
  const profile = await getStaffProfile(guildId, userId);
  const before = profile.warnings.length;
  profile.warnings = profile.warnings.filter((warning) => warning.id !== warningId);
  data.members[userId] = profile;
  await saveStaffData(guildId, data);
  return before !== profile.warnings.length;
}

export async function addPromotion(guildId, userId, record) {
  const data = await getStaffData(guildId);
  const profile = await getStaffProfile(guildId, userId);
  ...
  data.members[userId] = profile;
  await saveStaffData(guildId, data);
  return profile.promotions.at(-1);
}

export async function addDemotion(guildId, userId, record) {
  const data = await getStaffData(guildId);
  const profile = await getStaffProfile(guildId, userId);
  profile.demotions.push({
  ...
  data.members[userId] = profile;
  await saveStaffData(guildId, data);
  return profile.demotions.at(-1);
}

export async function addStaffNote(guildId, userId, authorId, note) {
  const data = await getStaffData(guildId);
  const profile = await getStaffProfile(guildId, userId);
  profile.notes.push({
    id: `sn_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
  ...
  data.members[userId] = profile;
  await saveStaffData(guildId, data);
  return profile.notes.at(-1);
}

export async function incrementStaffActivity(guildId, userId, type, amount = 1) {
  const data = await getStaffData(guildId);
  const profile = await getStaffProfile(guildId, userId);
  profile.activity = profile.activity || {};
  profile.activity[type] = Math.max(0, Number(profile.activity[type] || 0) + Number(amount || 0));
...
export function calculateActivityScore(profile) {
  ...
  const score = Math.min(100, moderation * 0.4 + tickets * 0.35 + events * 5 + commands * 0.05);
