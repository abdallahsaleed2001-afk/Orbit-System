...
import {
  ...
  getCustomTriggers,
  removeCustomTrigger,
} from '../../services/customTriggerService.js';
...
export default {
  data: new SlashCommandBuilder()
    .setName('trigger')
    .setDescription('Create and manage word-based actions.')
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
    .addSubcommand((subcommand) => subcommand
      .setName('add')
      .setDescription('Create or update a word trigger.')
      .addStringOption((option) => option
        .setName('word')
        .setDescription('The exact word or phrase to listen for.')
        .setRequired(true)
        .setMaxLength(100))
      .addStringOption((option) => option
        .setName('action')
        .setDescription('What Orbit should do.')
        .setRequired(true)
        .addChoices(
      ...
      .addRoleOption((option) => option
        .setName('role')
        .setDescription('Role to give/remove. Required for role actions.')
        .setRequired(false)))
    .addSubcommand((subcommand) => subcommand
      .setName('remove')
      .setDescription('Remove a word trigger.')
      .addStringOption((option) => option
        .setName('word')
    ...
    .addSubcommand((subcommand) => subcommand
      .setName('list')
      .setDescription('Show all configured word triggers.')),
  ...
  async execute(interaction, config, client) {
    ...
    try {
      if (subcommand === 'add') {
        ...
        if ((action === TRIGGER_ACTIONS.ADD_ROLE || action === TRIGGER_ACTIONS.REMOVE_ROLE) && !role) {
          return interaction.reply({ content: 'You must select a role for this action.', ephemeral: true });
        }

        if (role && role.managed) {
          return interaction.reply({ content: 'That role is managed by an integration and cannot be assigned by the bot.', ephemeral: true });
        }

        const botMember = interaction.guild.members.me;
        if (role && botMember && role.position >= botMember.roles.highest.position) {
          return interaction.reply({ content: 'I cannot manage that role because it is above my highest role.', ephemeral: true });
        }

        const entry = await addCustomTrigger(client, interaction.guild.id, word, action, role?.id || null);
        return interaction.reply({
          content: `Trigger saved: **${entry.trigger}** → **${ACTION_LABELS[entry.action]}**${entry.roleId ? ` → <@&${entry.roleId}>` : ''}`,
          ephemeral: true,
        });
      }
      ...
      if (subcommand === 'remove') {
        ...
        return interaction.reply({
          content: removed ? `Removed trigger **${word.trim()}**.` : `No trigger was found for **${word.trim()}**.`,
          ephemeral: true,
        });
      }
      ...
      if (!triggers.length) {
        return interaction.reply({ content: 'No custom triggers are configured.', ephemeral: true });
      }

      const lines = triggers.map((item) => {
        const role = item.roleId ? ` → <@&${item.roleId}>` : '';
        return `• **${item.trigger}** → ${ACTION_LABELS[item.action] || item.action}${role}`;
      });

      return interaction.reply({
        content: `**Custom Triggers (${triggers.length})**\n${lines.join('\n')}`.slice(0, 2000),
        ephemeral: true,
      });
    } catch (error) {
