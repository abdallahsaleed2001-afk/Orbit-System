...
export default {
    ...
    async execute(interaction) {
        ...
        // Use the members already available in cache.
        // This avoids Gateway Opcode 8 rate limits.
        const getAvailableMembers = () => {
            let memberArray = Array.from(interaction.guild.members.cache.values());

            memberArray = memberArray.filter(member => {
                if (member.user.bot && !includeBots) {
                    return false;
                }

                if (onlineOnly && member.presence?.status === 'offline') {
                    return false;
                }
        ...
        let memberArray = getAvailableMembers();
        ...
        collector.on('collect', async (i) => {
            try {
                let newMemberArray = getAvailableMembers();
                ...
                // Avoid selecting the same user again when possible.
                if (newMemberArray.length > 1) {
                    const differentUsers = newMemberArray.filter(
