...
function ensureMuteFile() {
  ...
  if (!fs.existsSync(MUTE_FILE)) fs.writeFileSync(MUTE_FILE, '{}', 'utf8');
}
...
function saveMutes(mutes) {
  ensureMuteFile();
  fs.writeFileSync(MUTE_FILE, JSON.stringify(mutes, null, 2), 'utf8');
}

function parseDuration(duration) {
  if (!duration) return null;
  const match = duration.toLowerCase().match(/^(\d+)\s*(s|m|h|d|w)$/);
...
async function scheduleUnmute(client, guildId, userId, expiresAt) {
  ...
  const unmute = async () => {
    try {
      ...
      const mutedRole = guild.roles.cache.find(role => role.name.toLowerCase() === MUTED_ROLE_NAME.toLowerCase());
      if (member && mutedRole && member.roles.cache.has(mutedRole.id)) {
        ...
        await logModerationAction({
          ...
          event: {
            ...
            metadata: { userId: member.id, moderatorId: client.user.id, automatic: true }
          }
        });
      }
    } catch (error) {
  ...
  if (delay <= 0) return unmute();
  setTimeout(unmute, delay);
}
...
async function restoreMutes(client) {
  ...
  for (const [key, data] of Object.entries(mutes)) {
    ...
    if (data?.expiresAt) await scheduleUnmute(client, guildId, userId, data.expiresAt);
  }
}
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('duration').setDescription('Mute duration: 10s, 10m, 2h, 1d, 1w. Leave empty for permanent.').setRequired(false))
  ...
  async execute(interaction, config, client) {
    ...
    if (member.id === interaction.guild.ownerId || member.roles.highest.position >= interaction.member.roles.highest.position) {
      return replyUserError(interaction, { type: ErrorTypes.PERMISSION, message: 'You cannot mute this member because their highest role is equal to or higher than yours.' });
    }
    if (mutedRole.position >= interaction.guild.members.me.roles.highest.position) {
      return replyUserError(interaction, { type: ErrorTypes.PERMISSION, message: 'My role must be above the `Muted` role.' });
    }

    let durationMs = null;
    if (duration) {
      durationMs = parseDuration(duration);
      if (!durationMs) return replyUserError(interaction, { type: ErrorTypes.UNKNOWN, message: 'Invalid duration. Use `10s`, `10m`, `2h`, `1d`, or `1w`.' });
    }

    try {
      await member.roles.add(mutedRole, `Muted by ${interaction.user.tag}`);
      const mutes = loadMutes();
      const muteKey = `${interaction.guild.id}:${member.id}`;
      const expiresAt = durationMs ? Date.now() + durationMs : null;
      if (expiresAt) {
        mutes[muteKey] = { guildId: interaction.guild.id, userId: member.id, expiresAt };
        saveMutes(mutes);
        await scheduleUnmute(client, interaction.guild.id, member.id, expiresAt);
      }
      ...
      const caseId = await logModerationAction({
        ...
        event: {
          ...
          metadata: { userId: member.id, roleId: mutedRole.id, roleName: mutedRole.name, expiresAt, moderatorId: interaction.user.id }
        }
      });
      ...
      await replyUserError(interaction, { type: ErrorTypes.PERMISSION, message: 'I could not give the Muted role to this member. Check my Manage Roles permission and role position.' });
    }
  },
