...
const MUTED_ROLE_ID = '1535481560172728402'; const MUTE_FILE = path.join(__dirname, '../../data/mutes.json');
...
export default {
  data: new SlashCommandBuilder().setName('mute').setDescription('Mutes a member using the Muted role.').addUserOption(option => option.setName('user').setDescription('The member to mute.').setRequired(true)).addStringOption(option => option.setName('duration').setDescription('Mute duration: 10s, 10m, 2h, 1d, 1w. Leave empty for permanent.').setRequired(false)).addStringOption(option => option.setName('reason').setDescription('Reason for the mute.').setRequired(false)).setDefaultMemberPermissions(PermissionFlagsBits.ModerateMembers),
  category: 'moderation',
  async execute(interaction, config, client) {
    const deferSuccess = await InteractionHelper.safeDefer(interaction); if (!deferSuccess) return;
    const user = interaction.options.getUser('user'), duration = interaction.options.getString('duration'), reason = interaction.options.getString('reason') || 'No reason provided'; const member = await interaction.guild.members.fetch(user.id).catch(() => null);
    if (!member) return replyUserError(interaction, { type: ErrorTypes.UNKNOWN, message: 'This user is not in the server.' });
    const mutedRole = interaction.guild.roles.cache.get(MUTED_ROLE_ID) || await interaction.guild.roles.fetch(MUTED_ROLE_ID).catch(() => null);
    if (!mutedRole) return replyUserError(interaction, { type: ErrorTypes.UNKNOWN, message: 'The configured Mute role could not be found.' });
    ...
    if (mutedRole.position >= interaction.guild.members.me.roles.highest.position) return replyUserError(interaction, { type: ErrorTypes.PERMISSION, message: 'My role must be above the Mute role.' });
    ...
    try {
      await member.roles.add(mutedRole, reason); const mutes = loadMutes(); const muteKey = `${interaction.guild.id}:${member.id}`; const expiresAt = durationMs ? Date.now() + durationMs : null;
      ...
      const caseId = await logModerationAction({ client, guild: interaction.guild, event: { action: 'Member Muted', target: `${member.user.tag} (${member.id})`, executor: `${interaction.user.tag} (${interaction.user.id})`, reason, duration: duration || 'Permanent', metadata: { userId: member.id, roleId: mutedRole.id, roleName: mutedRole.name, expiresAt, moderatorId: interaction.user.id } } });
      const durationText = durationMs ? `\nDuration: **${formatDuration(durationMs)}**` : '\nDuration: **Permanent**';
      await sendPunishmentDM({ user, guild: interaction.guild, type: 'mute', duration: durationMs ? formatDuration(durationMs) : null, reason, caseId });
    ...
    } catch (error) { logger.error('Mute command error:', error); await replyUserError(interaction, { type: ErrorTypes.PERMISSION, message: 'I could not give the Mute role to this member. Check my Manage Roles permission and role position.' }); }
  },
