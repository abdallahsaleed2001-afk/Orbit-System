...
import { getStaffData } from '../../services/staffService.js';
import {
  formatDuration,
  getShiftData,
  getShiftHistory,
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) => sub
      .setName('stop')
      .setDescription('Stop your current staff shift'))
    .addSubcommand((sub) => sub
      .setName('status')
      .setDescription('View a staff shift status')
      .addUserOption((opt) => opt.setName('user').setDescription('Staff member').setRequired(false)))
    .addSubcommand((sub) => sub
      .setName('history')
      .setDescription('View staff shift history')
      .addUserOption((opt) => opt.setName('user').setDescription('Staff member').setRequired(false))
      .addIntegerOption((opt) => opt.setName('limit').setDescription('Number of shifts to show').setMinValue(1).setMaxValue(20)))
    .addSubcommand((sub) => sub
      .setName('leaderboard')
      .setDescription('View the staff shift leaderboard'))
    .addSubcommand((sub) => sub
      .setName('setup')
      .setDescription('Configure staff shift requirements')
      .addNumberOption((opt) => opt.setName('minimum_hours').setDescription('Minimum required shift hours').setMinValue(0).setMaxValue(1000).setRequired(true))),
  category: 'Community',
  execute: withErrorHandling(async (interaction) => {
    ...
    if (sub === 'setup') {
      if (!hasManagerAccess(interaction, staffData)) {
        return interaction.reply({ content: 'You do not have permission to configure staff shifts.', ephemeral: true });
      }
    ...
    if (!isSelf && !managerAccess) {
      return interaction.reply({ content: 'You can only view your own shift information.', ephemeral: true });
    }

    if (sub === 'start') {
      if (!staffData.members[targetUser.id]) {
        return interaction.reply({ content: 'You are not registered in the staff system yet. Open your staff profile first.', ephemeral: true });
      }
