...
const clanCommand = new SlashCommandBuilder()
  .setName('clan')
  .setDescription('Manage clans and your clan')
  .setDMPermission(false)
  .addSubcommandGroup((group) => group
    .setName('admin')
    .setDescription('Administrator clan controls')
    .addSubcommand((sub) => sub
      .setName('create')
      .setDescription('Create a clan for a user')
...
async function fetchClanOwnerMember(interaction, clan) {
  return interaction.guild.members.fetch(clan.ownerId).catch(() => null);
}
  ...
  try {
    const overwrites = [
      ...
      },
      {
        ...
        allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.ManageChannels, PermissionFlagsBits.ManageMessages, PermissionFlagsBits.Connect, PermissionFlagsBits.Speak],
      },
    ];

    category = await guild.channels.create({
      name: `Clan • ${name}`.slice(0, 100),
      type: ChannelType.GuildCategory,
      permissionOverwrites: overwrites,
      reason: `Clan category for ${name}`,
    });
    ...
    textChannel = await guild.channels.create({
      ...
      parent: category.id,
      permissionOverwrites: overwrites,
      reason: `Clan text channel for ${name}`,
    });
    ...
    voiceChannel = await guild.channels.create({
      ...
      parent: category.id,
      permissionOverwrites: overwrites,
      reason: `Clan voice channel for ${name}`,
    });
    ...
    try {
      ...
      if (group === 'admin') {
        if (!isAdmin(interaction)) {
          return interaction.reply({ content: 'You need **Manage Server** permission to use clan administration.', ephemeral: true });
        }
        ...
        if (sub === 'create') {
          ...
          if (clans.some((clan) => clan.name.toLowerCase() === name.toLowerCase())) {
          ...
          const clan = await createClan(client, interaction.guildId, {
            id: makeClanId(),
            name,
            ownerId: owner.id,
        ...
        if (sub === 'info') {
          return interaction.reply({ embeds: [clanEmbed(clan)] });
        }

        if (sub === 'delete') {
          const channelIds = [clan.voiceChannelId, clan.textChannelId, clan.categoryId];
          for (const channelId of channelIds) {
            const channel = await guildChannel(interaction, channelId);
      ...
      const clan = await getClan(client, interaction.guildId, await resolveUserClanId(client, interaction.guildId, interaction.user.id));
      if (!clan) {
        return interaction.reply({ content: 'You are not a member of a clan.', ephemeral: true });
      }
      ...
      if (clan.ownerId !== interaction.user.id) {
        return interaction.reply({ content: 'Only the clan owner can manage clan members or rename the clan.', ephemeral: true });
      }
      ...
      if (sub === 'add') {
        ...
        const role = await interaction.guild.roles.fetch(clan.roleId).catch(() => null);
        if (role) await interaction.guild.members.addRole({ user: user.id, role: role.id, reason: `Added to clan ${clan.name}` }).catch(async () => {
      ...
      if (sub === 'rename') {
        ...
        const clans = await getClans(client, interaction.guildId);
        if (clans.some((item) => item.id !== clan.id && item.name.toLowerCase() === name.toLowerCase())) {
          return interaction.reply({ content: 'A clan with that name already exists.', ephemeral: true });
        }
...
async function resolveUserClanId(client, guildId, userId) {
  const clan = await getClanForUser(client, guildId, userId);
  return clan?.id || '';
}

async function guildChannel(interaction, channelId) {
