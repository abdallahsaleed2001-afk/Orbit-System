...
async function showMentionsMaxModal(i, client) {
      ...
      .addComponents(
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            ...
            .setValue(String(config.autoMod.mentions.max ?? 3))
...
export default [
  { name: 'security_panel_automod', execute: async (i, client) => ok(i) ? autoModPage(i, client) : deny(i) },
  { name: 'security_panel_automod2', execute: async (i, client) => ok(i) ? autoModPage(i, client) : deny(i) },
  { name: 'automod_global_toggle', execute: async (i, client) => { if (!ok(i)) return deny(i); const x = await getSecurityConfig(client, i.guildId); await updateSecurityConfig(client, i.guildId, { autoMod: { enabled: !x.autoMod.enabled } }); return autoModPage(i, client); } },
  { name: 'automod_rule', execute: async (i, client) => ok(i) ? rulePage(i, client, parseKey(i)) : deny(i) },
  { name: 'automod_back', execute: async (i, client) => ok(i) ? autoModPage(i, client) : deny(i) },
  { name: 'automod_toggle', execute: async (i, client) => { if (!ok(i)) return deny(i); const key = parseKey(i); const x = await getSecurityConfig(client, i.guildId); await updateSecurityConfig(client, i.guildId, { autoMod: { [key]: { enabled: !x.autoMod[key].enabled } } }); return rulePage(i, client, key); } },
  { name: 'automod_punishment', execute: async (i, client) => { if (!ok(i)) return deny(i); const key = parseKey(i); const x = await getSecurityConfig(client, i.guildId); const values = RULES[key].values; await updateSecurityConfig(client, i.guildId, { autoMod: { [key]: { punishment: cycle(x.autoMod[key].punishment, values) } }); return rulePage(i, client, key); } },
  { name: 'automod_limit', execute: async (i, client) => { if (!ok(i)) return deny(i); const key = parseKey(i); if (key === 'mentions') return showMentionsMaxModal(i, client); const x = await getSecurityConfig(client, i.guildId); const rule = x.autoMod[key]; const patch = key === 'spam' ? { maxMessages: rule.maxMessages >= 12 ? 3 : rule.maxMessages + 1 } : key === 'duplicate' ? { maxRepeats: rule.maxRepeats >= 8 ? 2 : rule.maxRepeats + 1 } : key === 'caps' ? { ratio: rule.ratio >= 0.95 ? 0.5 : Number((rule.ratio + 0.05).toFixed(2)) } : {}; await updateSecurityConfig(client, i.guildId, { autoMod: { [key]: patch } }); return rulePage(i, client, key); } },
  { name: 'automod_window', execute: async (i, client) => { if (!ok(i)) return deny(i); const key = parseKey(i); const x = await getSecurityConfig(client, i.guildId); const current = Math.round(x.autoMod[key].windowMs / 1000); const values = key === 'spam' ? [3, 5, 10, 15, 30] : [5, 10, 15, 30, 60]; const next = cycle(current, values); await updateSecurityConfig(client, i.guildId, { autoMod: { [key]: { windowMs: next * 1000 } } }); return rulePage(i, client, key); } },
  { name: 'automod_min', execute: async (i, client) => { if (!ok(i)) return deny(i); const key = parseKey(i); const x = await getSecurityConfig(client, i.guildId); const next = x.autoMod[key].minLength >= 30 ? 8 : x.autoMod[key].minLength + 2; await updateSecurityConfig(client, i.guildId, { autoMod: { [key]: { minLength: next } } }); return rulePage(i, client, key); } },
  { name: 'automod_words', execute: async (i, client) => ok(i) ? rulePage(i, client, 'badWords') : deny(i) },
];