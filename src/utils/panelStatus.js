/** Shared helpers for reaction-role panel status. */
...
export function formatPanelStatusField(panelStatus, { repostHint = 'Repost Panel' } = {}) {
    ...
    if (panelStatus.exists) return panelStatus.message?.url ? `✅ Active — [view panel](${panelStatus.message.url})` : '✅ Active';
    if (panelStatus.reason === 'channel_missing') return '⚠️ Panel channel missing or deleted';
    if (panelStatus.reason === 'panel_deleted') return `⚠️ Panel message was deleted — use **${repostHint}** below`;
    if (panelStatus.reason === 'no_channel') return '⚠️ No panel channel configured';
    return '`Unknown`';
}

export async function getBotPanelStatus(client, guild, { channelId, messageId = null, buttonCustomId = null, selectCustomId = null, scanLimit = 50 } = {}) {
    if (!channelId || (!buttonCustomId && !selectCustomId)) return { exists: false, reason: 'no_channel' };
    const channel = await guild.channels.fetch(channelId).catch(() => null);
    if (!channel) return { exists: false, reason: 'channel_missing' };
    const marker = { buttonCustomId, selectCustomId };
    if (messageId) {
        const message = await channel.messages.fetch(messageId).catch(() => null);
        if (message && messageHasPanelMarker(message, marker)) return { exists: true, message, channel };
    }
    const messages = await channel.messages.fetch({ limit: scanLimit }).catch(() => null);
    const messageList = messages ? [...(typeof messages.values === 'function' ? messages.values() : messages)] : [];
    const recovered = messageList.find(entry => entry.author.id === client.user.id && messageHasPanelMarker(entry, marker));
    if (recovered) return { exists: true, message: recovered, channel, recoveredId: recovered.id };
