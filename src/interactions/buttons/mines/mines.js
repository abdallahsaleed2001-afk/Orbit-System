...
export function grid(game) {
  const buttons = [];
  for (let i = 0; i < 9; i++) buttons.push(new ButtonBuilder().setCustomId(`mines_cell:${game.guildId}:${game.channelId}:${i}`).setLabel(game.revealed.has(i) ? '✓' : '■').setStyle(game.revealed.has(i) ? ButtonStyle.Success : ButtonStyle.Secondary).setDisabled(game.revealed.has(i) || !game.active));
  return [0,1,2].map(r => new ActionRowBuilder().addComponents(...buttons.slice(r * 3, r * 3 + 3)));
}
function players(game) { return game.players.length ? game.players.map(p => `<@${p.id}>`).join(' × ') : 'لا يوجد لاعبين'; }
function content(game, extra = '') { const turn = currentPlayer(game); return `**INFINITY GAMES — لغم**\n\n${extra ? `${extra}\n\n` : ''}اللاعبون: ${players(game)}\n\nالجولة: **#${game.round}**\nدور: ${turn ? `<@${turn.id}>` : '—'}\nلغم واحد مخفي في هذه الجولة.\nلديك **10 ثوانٍ** للتفاعل.`; }
...
export function startMinesTurnTimer(channel, game) { return startTurnTimer(channel, game); }

async function executeMinesButton(interaction) {
  const parts = interaction.customId.split(':');
  const action = parts[0], guildId = parts[1], channelId = parts[2], cell = parts[3] === undefined ? null : Number(parts[3]);
  ...
  if (action === 'mines_join') {
    ...
    if (result.error === 'inactive') return interaction.reply({ content: 'انتهى وقت الدخول.', ephemeral: true });
    ...
    return interaction.update({ content: `**INFINITY GAMES — لغم**\n\nالتسجيل مفتوح.\nاللاعبون (**${game.players.length}**): ${players(game)}\n\nوقت الدخول: <t:${game.joinEndsAt}:R>`, components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`mines_join:${guildId}:${channelId}`).setLabel('انضم').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`mines_leave:${guildId}:${channelId}`).setLabel('خروج').setStyle(ButtonStyle.Secondary))] });
  }
  if (action === 'mines_leave') {
    ...
    if (game.phase === 'join') {
      if (result.empty) { endMines(game); return interaction.update({ content: 'تم إلغاء اللعبة لعدم وجود لاعبين.', components: [] }); }
      return interaction.update({ content: `**INFINITY GAMES — لغم**\n\n${interaction.user} خرج من التسجيل.\nاللاعبون (**${game.players.length}**): ${players(game)}\n\nوقت الدخول: <t:${game.joinEndsAt}:R>`, components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`mines_join:${guildId}:${channelId}`).setLabel('انضم').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`mines_leave:${guildId}:${channelId}`).setLabel('خروج').setStyle(ButtonStyle.Secondary))] });
    }
    clearTimer(game);
    if (game.players.length <= 1) { const winner = game.players[0]; endMines(game); return interaction.update({ content: `**INFINITY GAMES — لغم**\n\n${interaction.user} خرج من اللعبة.\n\n🏆 الفائز: ${winner ? `<@${winner.id}>` : 'لا يوجد'}`, components: [] }); }
  ...
  if (action === 'mines_cell') {
    ...
    clearTimer(game);
    if (result.mine) {
      if (result.finished) { endMines(game); return interaction.update({ content: `**INFINITY GAMES — لغم**\n\n💣 تم طرد <@${result.eliminated.id}>\n\n🏆 الفائز: <@${result.winner.id}>`, components: [] }); }
      await interaction.update({ content: content(game, `💣 تم طرد <@${result.eliminated.id}> — تم الانتقال إلى الجولة **#${game.round}**.`), components: grid(game) });
      startTurnTimer(interaction.channel, game);
      return;
    }
