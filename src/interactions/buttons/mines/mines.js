...
export function grid(game) {
  const buttons = [];
  for (let i = 0; i < 9; i++) {
    buttons.push(new ButtonBuilder().setCustomId(`mines_cell:${game.guildId}:${game.channelId}:${i}`).setLabel(game.revealed.has(i) ? '✓' : '■').setStyle(game.revealed.has(i) ? ButtonStyle.Success : ButtonStyle.Secondary).setDisabled(game.revealed.has(i) || !game.active));
  }
