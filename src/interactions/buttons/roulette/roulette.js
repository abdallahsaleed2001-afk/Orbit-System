...
function buildRows(game) {
  ...
  if (game.phase === 'join') {
    rows.push(new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`roulette_join:${game.id}`).setLabel('🎟️ انضم').setStyle(ButtonStyle.Success),
  ...
  const targetButtons = game.participants.map(p => new ButtonBuilder()
    .setCustomId(`roulette_target:${game.id}:${p.id}`)
    .setLabel(p.username.slice(0, 80))
    .setStyle(p.id === selected ? ButtonStyle.Secondary : ButtonStyle.Primary)
    .setDisabled(p.id === selected));
  for (let i = 0; i < targetButtons.length && rows.length < 4; i += 5) rows.push(new ActionRowBuilder().addComponents(...targetButtons.slice(i, i + 5)));
  rows.push(new ActionRowBuilder().addComponents(
...
function buildEmbed(game, title = '🎡 الروليت') {
  ...
  const embed = new EmbedBuilder()
    .setColor(0x5865f2)
    .setTitle(title)
    .setDescription(`**المشاركون (${game.participants.length})**\n${participantText(game)}`)
    .setFooter({ text: game.phase === 'join' ? 'اضغط انضم للمشاركة.' : 'الدور للاعب الذي توقفت عليه العجلة.' });
  if (selected) embed.setAuthor({ name: `الدور على ${selected.username}`, iconURL: selected.avatar });
  return embed;
}

function payload(game, gifBuffer = null) {
  const files = gifBuffer ? [new AttachmentBuilder(gifBuffer, { name: 'roulette.gif' })] : [];
  const embed = buildEmbed(game);
  if (gifBuffer) embed.setImage('attachment://roulette.gif');
  return { embeds: [embed], components: buildRows(game), files };
}

export async function sendJoinMessage(message, game) {
  const sent = await message.channel.send({
    content: `🎡 **بدأت الروليت!** ${message.author}\nاضغط **انضم** للدخول. لديك **12 ثانية**.`,
    ...payload(game),
  });
  ...
  game.onJoinTimeout = async () => {
    ...
    if (game.participants.length < 2) {
      await sent.edit({ content: '❌ **انتهت الروليت** — يجب أن يشارك شخصان على الأقل.', components: [], embeds: [buildEmbed(game, '🎡 الروليت — انتهت')] }).catch(() => {});
...
export async function spinRound(message, game) {
  ...
  await message.edit({ content: `🎡 **العجلة تدور...**`, ...payload({ ...game, phase: 'spinning' }, gif) }).catch(() => {});
  ...
  const timer = setTimeout(async () => {
    ...
    game.phase = 'decision';
    await message.edit({
      content: `🎯 **توقفت العجلة على:** <@${result.participant.id}>`,
      ...payload(game),
    }).catch(() => {});
  }, 3600);
...
export async function handleRouletteButton(interaction, client, args) {
  ...
  if (action === 'stop') {
    ...
    cancelRoulette(game.guildId, game.channelId);
    await interaction.update({ content: '🛑 **تم إيقاف الروليت.**', embeds: [], components: [], files: [] });
    return;
  }
  ...
  if (action === 'target') {
    ...
    chooseRandomTarget(game, interaction.user.id);
  }
  ...
  if (game.phase === 'finished') {
    ...
    await interaction.update({
      content: `🏆 **الفائز في الروليت:** <@${winner.id}>`,
      embeds: [buildEmbed(game, '🏆 الروليت — انتهت')],
      components: [],
      files: [],
    });
    return;
